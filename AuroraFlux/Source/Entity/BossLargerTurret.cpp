...
* Mod. Initials:	AR
*****************************************************************/
CBossLargerTurret::CBossLargerTurret(CEntity * _pOwner, CEntity * _pTarget, int _type, int _weaponType, float _zModifier) : m_pOwner(_pOwner), m_nTurretType(_type)
{
	m_nWeaponType	 = _weaponType;
	m_nObjectType   = eTURRET_LASER;
	m_nHealth       = 100;
	m_nShields      = 0;
...
/*Destructor*/
CBossLargerTurret::~CBossLargerTurret(void)
{
...
* Mod. Initials:	AR
*****************************************************************/
void CBossLargerTurret::Update(float _fElapedTime)
{
	...
	if (corretSide)
	{
		...
		{
			CPhysics::TurnTo(this, m_pTarget, _fElapedTime, TURRE_LARGE_TURN_RATE);
		}
	}
...
bool CBossLargerTurret::TopTurret(float _fElapsedTime)
{
	...
	{
		if (m_fCounter >=  TURRE_LARGE_FIRE_RATE_LASER )
		{
			bCanFire = true;
...
// 		D3DXVec3Normalize(  &d3dYAxis, &d3dYAxis );
		// on the right side?
		//if(D3DXVec3Dot( &d3dVectorBetween, &d3dYAxis)  > TURRE_LARGE_SIGHT && sqDist <= BOSS_TURRET_RANGE )  
		if (((CBoss*)m_pOwner)->IsOnTop())
		{
			if (bCanFire)
			{
...
bool CBossLargerTurret::BottomTurret(float _fElapsedTime)
{
	...
	if(m_nWeaponType == eTURRET_LASER)
	{
		if (s_fCounter >=  TURRE_LARGE_FIRE_RATE_LASER )
		{
			bCanFire = true;
			s_fCounter = 0.0f;
...
bool CBossLargerTurret::LeftTurret(float _fElapsedTime)
{
	static float s_fCounter = 0.0f;
	m_fCounter += _fElapsedTime;
	bool bCanFire = false;
...
// 		D3DXVec3Normalize(  &d3dXAxis, &d3dXAxis );
		// on the right side?
		//if(D3DXVec3Dot( &d3dVectorBetween, &d3dXAxis)  < -TURRE_LARGE_SIGHT && sqDist <= BOSS_TURRET_RANGE )  
...
bool CBossLargerTurret::RightTurret(float _fElapsedTime)
{
...
bool CBossLargerTurret::FrontTurret(float _fElapsedTime)
{
...
//}
	...
	if (m_pTarget->GetPosition().z < GetPosition().z)
	{
		...
		// on the right side?
		if(D3DXVec3Dot( &d3dVectorBetween, &d3dZAxis)  > TURRE_LARGE_SIGHT && sqDist <= BOSS_TURRET_RANGE)  
		{
			if (bCanFire)
			{
				if(m_nWeaponType == eTURRET_LASER)
...
bool CBossLargerTurret::BackTurret(float _fElapsedTime)
{
	static float s_fCounter = 0.0f;
	s_fCounter += _fElapsedTime;
	bool bCanFire = false;
